  ...
  return (
    <Card className="relative overflow-hidden">
      <div className="relative aspect-[9/16] md:aspect-video">
        <img
          src={thumbnailUrl || 'https://images.unsplash.com/photo-1587825140708-dfaf72ae4b04?w=800'}
          alt={title}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 md:p-6">
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm text-muted-foreground">with {instructorName}</span>
            <Button 
              size="sm" 
              variant={status === 'live' ? 'default' : 'secondary'}
              onClick={() => window.location.href = `/live/${eventId}`}
            >
              {status === 'live' ? 'Join Now' : status === 'scheduled' ? 'Register' : 'Watch'}
            </Button>
          </div>
        </div>
      </div>
