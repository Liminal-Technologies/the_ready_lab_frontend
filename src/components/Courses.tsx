...
const Courses = () => {
  ...
  const getLevelColor = (level: string) => {
    switch (level) {
      case "Beginner":
        return "bg-green-100/30 text-green-700";
      case "Intermediate":
        return "bg-orange-100/30 text-orange-700";
      case "Advanced":
        return "bg-red-100/30 text-red-700";
      default:
        return "bg-blue-100/30 text-blue-700";
    }
  };
  ...
  return (
    <section
      id="courses"
      className="py-20 lg:py-32 bg-white transition-colors duration-200"
    >
      <div className="container mx-auto px-4 lg:px-8 max-w-7xl">
        {/* Header */}
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-orange-100/30 text-orange-600 px-4 py-2 rounded-full text-sm font-semibold mb-6">
            <TrendingUp className="h-4 w-4" />
            {t("courses.certificationTracks")}
          </div>
          <h2 className="text-5xl md:text-6xl lg:text-7xl font-black mb-6">
            <span className="text-black">Get Fundable & </span>
            <span className="text-[#D97706]">Certified</span>
          </h2>
        ...
        <div className="bg-orange-50/20 rounded-[3rem] p-8 lg:p-12 mb-12">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {courses.map((course) => (
              <div
                key={course.id}
                className="bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer flex flex-col"
              ...
              >
                ...
                <div className="relative h-52 overflow-hidden bg-neutral-100">
                  ...
                  />
                  <div className="absolute top-4 right-4 flex items-center gap-1 bg-yellow-400 text-black px-3 py-1.5 rounded-full font-bold text-lg shadow-lg">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <div className="flex items-center gap-2 flex-wrap mb-4">
                    <Badge
                      className={`${getLevelColor(course.level)} border-0 text-xs font-semibold px-3 py-1`}
                    >
                      {course.level}
                    </Badge>
                    {course.certification && (
                      <Badge className="bg-blue-100/30 text-blue-700 border-0 text-xs font-semibold px-3 py-1">
                        <Award className="h-3 w-3 mr-1" />
                        Certified
                      </Badge>
                    )}
                    {course.featured && (
                      <Badge className="bg-yellow-100/30 text-yellow-700 border-0 text-xs font-semibold px-3 py-1">
                        Featured
                      </Badge>
                    )}
                  </div>
                  ...
                  {/* Category */}
                  <p className="text-xs font-bold text-neutral-500 mb-2 uppercase tracking-wide">
                    {course.category}
                  </p>
                  ...
                  {/* Description */}
                  <p className="text-sm lg:text-base text-neutral-600 mb-4 flex-grow leading-relaxed">
                    {course.description}
                  </p>
                  ...
                  <Button
                    className={`w-full rounded-full font-semibold ${
                      course.featured
                        ? "bg-black text-white hover:bg-black/80:bg-white/90"
                        : "bg-white text-black border-2 border-black hover:bg-black hover:text-white:bg-white:text-black"
                    }`}
                    onClick={(e) => {
                      e.stopPropagation();
        ...
        {/* Bottom CTA Bar */}
        <div className="bg-gradient-to-r from-yellow-400 to-orange-500 rounded-3xl p-8 shadow-xl text-center">
          <Button
            size="lg"
          ...
          <p className="text-sm text-black/70/80 mt-4 font-medium">
