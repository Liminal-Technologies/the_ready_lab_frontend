...
import { Heart, MessageCircle, Bookmark, Share2, MoreVertical, Pin, Trash2, Flag } from 'lucide-react';
...
export const PostCard = ({ post, currentUserId, isModerator, onPostDeleted, onPostPinned }: PostCardProps) => {
  ...
  const handlePin = async () => {
    try {
      ...
      toast.success(post.is_pinned ? "Post unpinned" : "Post pinned", {
        description: post.is_pinned ? "Post removed from top" : "Post pinned to top of community",
      });
    ...
    <Card className="card-interactive">
      <CardHeader>
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <Avatar>
              <AvatarFallback>
                {(post.profiles?.full_name || post.profiles?.email || 'U').charAt(0).toUpperCase()}
              </AvatarFallback>
            </Avatar>
            <div>
      ...
      <CardContent className="space-y-4">
        ...
        {post.link_url && (
          ...
          >
            {post.link_image && (
              <img src={post.link_image} alt={post.link_title || 'Link'} className="w-full h-48 object-cover rounded mb-2" />
            )}
        ...
        <div className="flex items-center justify-between pt-4 border-t">
          <div className="flex items-center gap-4">
            <PostReactions postId={post.id} currentUserId={currentUserId} initialCount={post.likes_count} />
