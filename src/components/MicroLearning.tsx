...
import { Heart, MessageCircle, Share2, BookOpen, Clock, TrendingUp, Star, Award } from "lucide-react";
              ...
              <CardContent className="p-6">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-neutral-900 dark:text-white">{lesson.author}</p>
                      <p className="text-sm text-neutral-600 dark:text-neutral-400">{lesson.time}</p>
                    </div>
                  </div>
                ...
                {/* Content */}
                <div className="mb-4">
                  <h3 className="font-semibold text-neutral-900 dark:text-white mb-2">{lesson.title}</h3>
                  <p className="text-neutral-700 dark:text-neutral-200 whitespace-pre-line leading-relaxed">{lesson.content}</p>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-neutral-200 dark:border-neutral-700">
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      className="flex items-center gap-2 text-neutral-600 dark:text-neutral-400 hover:text-yellow-500 transition-all duration-200"
                    >
                      <Heart 
                        className={`h-5 w-5 ${likedPosts.has(lesson.id) ? 'fill-current text-red-500' : ''}`} 
                      />
                      <span className="text-sm">{lesson.likes + (likedPosts.has(lesson.id) ? 1 : 0)}</span>
                    </button>
                    <button className="flex items-center gap-2 text-neutral-600 dark:text-neutral-400 hover:text-yellow-500 transition-all duration-200">
