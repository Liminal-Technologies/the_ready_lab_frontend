...
import { generateVideoThumbnail, uploadThumbnail } from '@/utils/thumbnailGenerator';
...
export const CreateMicrolearningModal = ({ open, onOpenChange, onSuccess }: CreateMicrolearningModalProps) => {
  ...
  const handleVideoUploaded = async (videoUrl: string, duration: number, blob?: Blob) => {
    setFormData({ ...formData, content_url: videoUrl, video_duration_seconds: duration });
    if (blob) {
      setVideoBlob(blob);
      ...
      try {
        ...
        const thumbnailUrl = await uploadThumbnail(thumbnailBlob, user.id, Date.now().toString(), supabase);
            ...
            <div className="space-y-4">
              <div className="p-4 bg-muted rounded-lg">
                ...
                <p className="text-xs text-muted-foreground">
                  Duration: {Math.floor(formData.video_duration_seconds / 60)}:{(formData.video_duration_seconds % 60).toString().padStart(2, '0')}
                </p>
              </div>
