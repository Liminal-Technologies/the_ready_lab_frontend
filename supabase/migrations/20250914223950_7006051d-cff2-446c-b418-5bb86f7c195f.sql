...
-- Create some initial communities
INSERT INTO public.communities (name, description, visibility, created_by) VALUES
('Funding Mastery', 'Connect with entrepreneurs on their funding journey', 'open', (SELECT id FROM public.profiles WHERE role = 'admin' LIMIT 1)),
('Brand Builders', 'Share branding insights and get feedback on your visual identity', 'open', (SELECT id FROM public.profiles WHERE role = 'admin' LIMIT 1)),
('AI Innovators', 'Discuss the latest AI tools and implementation strategies', 'open', (SELECT id FROM public.profiles WHERE role = 'admin' LIMIT 1));